...
SERVICE_NAME="${BABBLEDECK_HEALTH_SERVICE:-aialra-babbledeck-health-monitor.service}"
TIMER_NAME="${BABBLEDECK_HEALTH_TIMER:-aialra-babbledeck-health-monitor.timer}"
UNIT_DIR="${BABBLEDECK_SYSTEMD_UNIT_DIR:-/etc/systemd/system}"
ON_CALENDAR="${BABBLEDECK_HEALTH_ON_CALENDAR:-*:0/5}"
RANDOMIZED_DELAY="${BABBLEDECK_HEALTH_RANDOMIZED_DELAY:-30s}"
...
[Unit]
Description=AIALRA BabbleDeck production health monitor
...
ExecStart=/usr/bin/bash $APP_DIR/scripts/monitor-production-health.sh
...
cat >"$timer_tmp" <<UNIT
[Unit]
Description=Every 5 minutes AIALRA BabbleDeck production health monitor
