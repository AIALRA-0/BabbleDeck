...
- Commands:
  ...
  - `pnpm --filter @babbledeck/web test -- AndroidApkInstallQr device-runtime-evidence kit`
...
- Results:
  ...
  - Added login-gated Android, iOS, and desktop wrapper handoff pages, release verification link creation on those pages, and kit handoff fields so native runtime handoff can start from production Settings without making wrapper artifacts public.
  - Updated the release-bound checklist and device readiness output so Android, iOS, and desktop operators start from the protected handoff pages.
  - Added stable non-secret receipt IDs to future device evidence records and receipts so operators can reference the exact JSONL record after real verification.
