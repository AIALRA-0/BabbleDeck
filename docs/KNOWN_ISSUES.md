...
- `pnpm device:readiness:production` currently reports a built Android debug APK but no connected physical Android device, no macOS/Xcode host for iOS, and no interactive desktop display session; production readiness also lacks `pnpm device:evidence:production` JSONL evidence for Android, iOS, and desktop runs.
