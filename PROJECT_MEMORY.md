...
- After the latest production deploy, `https://babbledeck.aialra.online` reports Soniox and LiveKit configured, web/recorder WS/LiveKit services active with `NRestarts=0`, Soniox recorder and UI smokes passed after the newly added key was present, and strict live readiness has all required checks passing while the only remaining external failure is local audio storage.
