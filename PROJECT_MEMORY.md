...
- Recorder pages expose backup reconnect, pending-chunk retry, and uploaded-chunk cleanup controls; Playwright seeds local IndexedDB chunks and verifies failed chunks replay while uploaded local copies can be removed without deleting pending recovery data.
...
- Production Soniox long trace can be run with `pnpm soniox:trace:production`; it generates a longer speech WAV, runs the deployed recorder/viewer UI, verifies persisted transcript segments/translations/audio chunks/provider usage/provider errors, archives the trace session, writes a non-secret JSONL marker, and is now part of required readiness evidence.
...
- Admin settings management for persisted default session language/budget, glossary CRUD, and read-only audit logs is deployed to production; production desktop/mobile Playwright main flows passed after deployment, and recent smoke test sessions were archived after validation.
- Viewer caption controls for translation-only, bilingual, and original-only modes, visible-transcript copy, caption sizing, and light/dark theme switching are deployed to production and covered by desktop/mobile Playwright main flows.
- Recorder microphone input health feedback for no-input and clipping states is deployed to production and covered by Playwright with controlled silent and high-gain browser audio streams.
- Recorder uploaded-local-backup cleanup is deployed to production: the recorder can delete only successfully uploaded IndexedDB chunks after recording, failed/pending chunks remain recoverable, and production Playwright passed both the cleanup-specific desktop test and the desktop/mobile main flow.
...
- Production native/device runtime evidence can now be recorded with `pnpm device:evidence:production` after real Android, iOS, or desktop wrapper runs. Strict production readiness now includes external check `recent_device_runtime_evidence`, which requires fresh passing Android, iOS, and desktop evidence before `productionReady=true`.
