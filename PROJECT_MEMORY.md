...
- LiveKit V2 has an optional room-audio foundation: `livekit-server-sdk` creates short-lived room tokens, `/api/sessions/:id/livekit-token` issues recorder/admin publisher tokens, `/api/viewer/session/:shareToken/livekit-token` issues viewer subscriber tokens, `livekit-client` publishes the recorder microphone track and subscribes viewer audio when configured, and health/settings expose only non-secret configured status. Production currently reports LiveKit `configured=false`; token endpoints and browser room-audio UI degrade to not-configured states until LiveKit server credentials and a running LiveKit deployment are supplied.
- Production LiveKit env setup can now use `pnpm livekit:configure:production`; it prepares a patched env from the current shell, runs `pnpm livekit:preflight:production` against the temporary env by default, and only installs the env after token grant and management API checks pass.
...
- After the latest production deploy, the configured Soniox API key passed `/api/health`, strict live Soniox readiness, the recorder WebSocket smoke, and the Chromium fake-microphone UI smoke on `https://babbledeck.aialra.online`; GitHub Actions for commit `19f77c7` also passed.
