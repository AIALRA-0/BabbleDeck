...
- LiveKit V2 has an optional server-side foundation deployed to production: `livekit-server-sdk` creates short-lived room tokens, `/api/sessions/:id/livekit-token` issues recorder/admin publisher tokens, `/api/viewer/session/:shareToken/livekit-token` issues viewer subscriber tokens, and health/settings expose only non-secret configured status. Production currently reports LiveKit `configured=false`; the token endpoint returns `PROVIDER_NOT_CONFIGURED` until LiveKit server credentials and a running LiveKit deployment are supplied.
...
- After the latest production deploy, the newly configured Soniox API key passed `/api/health`, strict live Soniox readiness, the recorder WebSocket smoke, and the Chromium fake-microphone UI smoke on `https://babbledeck.aialra.online`; GitHub Actions for commit `d49c8b6` also passed.
