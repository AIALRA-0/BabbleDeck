...
- LiveKit V2 has an optional server-side foundation: `livekit-server-sdk` creates short-lived room tokens, `/api/sessions/:id/livekit-token` issues recorder/admin publisher tokens, `/api/viewer/session/:shareToken/livekit-token` issues viewer subscriber tokens, and health/settings expose only non-secret configured status. Actual local room publish/subscribe still needs LiveKit server credentials and a running LiveKit deployment.
